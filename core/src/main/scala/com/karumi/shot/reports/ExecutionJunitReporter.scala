...
class ExecutionJunitReporter extends ExecutionReporter {
  ...
  private def writeReport(
    ...
    val indexFile = new File(reportFolder + "JUnit.xml")
  ...
  private def resetVerificationReport(shotFolder: ShotFolder) = {
    val file = new File(shotFolder.reportFolder() + "JUnit.xml")
    if (file.exists()) {
      file.delete()
    }
  }
